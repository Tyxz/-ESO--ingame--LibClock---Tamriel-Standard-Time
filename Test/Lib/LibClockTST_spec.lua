...
describe("LibClockTST", function()
    describe("Utility", function()
        it("should return true if object is not nil or empty", function()
            local tEmpty = "   "
            local tNil = nil
            local tString = "Test"

            assert.is_true(LibClockTST.IsNotNilOrEmpty(tString))
            assert.is_false(LibClockTST.IsNotNilOrEmpty(tEmpty))
            assert.is_false(LibClockTST.IsNotNilOrEmpty(tNil))
        end)

        it("should return true if is a timestamp", function()
            local tStamp = 1581865278
            local tShort = 0002
            local tLong = 12312455663693185718235
            local tString = "Test"
            local tNil = nil
            assert.is_true(LibClockTST.IsTimestamp(tStamp))
            assert.is_false(LibClockTST.IsTimestamp(tShort))
            assert.is_false(LibClockTST.IsTimestamp(tLong))
            assert.is_false(LibClockTST.IsTimestamp(tString))
            assert.is_false(LibClockTST.IsTimestamp(tNil))
        end)

        it("should return true if year is leap year", function()
            local tLeap = 2004
            local tNotLeap = 1900
            assert.is_true(LibClockTST.IsLeapYear(tLeap))
            assert.is_false(LibClockTST.IsLeapYear(tNotLeap))
        end)

        it("should return the current week day", function()
            local tTable = {
                year = 2020,
                month = 2,
                day = 16,
            ...
            local tWeekDay = 7
            local tResult = LibClockTST.GetWeekDay(tTable)
            assert.is.equal(tWeekDay, tResult)
        end)
    end)
